...
/* 	 works more efficient than classmates
 * 						Easy:								Medium:									Hard:
		 		Max Size of Queue: 			16		Max Size of Queue:		  88			Max Size of Queue:			 22,857		
				Nodes Explored:				25		Nodes Explored:			  134			Nodes Explored:				180,866
				# Steps in Solution:		5		# Steps in Solution:	  9				# Steps in Solution:		     30
				Cost of Solution:			17		Cost of Solution:		  31			Cost of Solution:				128
*/

// expands nodes by (costOfPath)

public class UniformCost_EightPuzzle {
	...
	Map <String, PuzzleState> 	unexplored;										// a copy of our unexplored moves that will allow us to get to data 
	...
	UniformCost_EightPuzzle(PuzzleState initialState, String goal){				// constructor
		this.stepsToSolve = new LinkedList<PuzzleState>();
		this.unexploredMoves = new PriorityQueue<PuzzleState>();
		this.unexplored = new HashMap<String, PuzzleState>();
	...
	void getSuccessors(){														// discovers legal moves and adds them to unexploredMoves if they are new
		...
		while (!unexploredMoves.isEmpty()){
			...
			explored.put(current.tilePos, current);								// marks current PuzzleState as explored exploredMoves
	...
	boolean iShouldAdd(PuzzleState newState){									// determines if adding a puzzleState is appropriate
		...
		if (unexploredMoves.contains(newState)){
			int prevCost = unexplored.get(newState).costOfPath;
			if (prevCost > newState.costOfPath){
				unexplored.remove(newState.tilePos);
				unexploredMoves.remove(newState);
			}
		}
	...
	void  addToUnexploredMoves(PuzzleState newState){							// adds PuzzleState node to unexploredMoves if it is the lowest cost instance 
		...
		unexplored.put(newState.tilePos, newState);
