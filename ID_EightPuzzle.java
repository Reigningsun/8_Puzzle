...
// expands nodes by (LIFO) only looking to a depth limit then searching a lower depth if a solution isn't found

public class ID_EightPuzzle {
	...
	ID_EightPuzzle(PuzzleState initialState, String goal){						// constructor
	...
	boolean getSuccessors(int depthLimit){										// discovers legal moves and adds them to unexploredMoves if they are new
		...
		while (!unexploredMoves.isEmpty()){
			...
			if (blanksIndex != 2 && blanksIndex != 5 && blanksIndex != 8){		// LEFT: move adjacent tile into blank space by sliding it LEFT
				...
				PuzzleState newState = new PuzzleState(newTilePos, current, 	// create PuzzleState representing the game state after taking a LEFT move
															cost, "Left");		
				if (iShouldAdd(newState, depthLimit)){ 							// adds new state to unexplored queue if it isn't already on there
	...
	boolean iShouldAdd(PuzzleState newState, int depthLimit){							// determines whether a PuzzleState should be added
		if (!unexploredMoves.contains(newState) 
				&& !exploredMoves.containsKey(newState.tilePos)
				&& newState.depth < depthLimit){										// depth limit 
			return true;
		}
		return false;
	}
	...
	boolean endSearchOutputSolution(PuzzleState current){						// update and print current solutionPath							
